package me.wavetech.wavecarpet.mixins.command.player.startFallFlying;
...
public class PlayerCommandMixin {
	...
	private static <T> RequiredArgumentBuilder<CommandSourceStack, T> insertStartFallFlyingParameter(RequiredArgumentBuilder<CommandSourceStack, T> original) {
		return original.then(literal("startFallFlying").executes(context -> {
			...
			context.getSource().sendSuccess(
				() -> Component.literal(
					"Started fall flying for " + player.getDisplayName().getString()
				),
				false
			);
			return 1;
